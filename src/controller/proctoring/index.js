...
class ProctoringController {
  ...
  async getAllProctoringResult(req, res) {
    const { page, pageSize } = req.query;
    const result = await ProctoringService.getAllProctoringReult(
      parseInt(page),
      parseInt(pageSize)
    );
    if (result.status) {
  ...
  async updateProctoringData(req, res) {
    const result = await ProctoringService.updateProctoringData(
      req.params.id,
      req.body
    );
    if (result.status) {
